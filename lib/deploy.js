...
function deployFromDirectory(packageDirectoryPath, options) {
  var dirname = path.basename(packageDirectoryPath);
  var cwd = path.join(packageDirectoryPath, '..');
  var archive = archiver('zip');
...
function reportDeployResult(res, logger, verbose) {
  var message =
    res.success ? 'Deploy Succeeded' + (res.status === 'SucceededPartial' ? ' Partially.' : '.') :
    ...
    if (testResult && Number(testResult.numTestsRun) > 0) {
      ...
      if (verbose) {
        ...
        testSuccesses.forEach(function(s) {
          logger.log(' - ' + (typeof s.namespace === 'string' ? s.namespace + '__' : '') + s.name + '.' + s.methodName);
        });
      }
