...
function deploy() {
  ...
  (
    program.archiveFile ? tools.deployFromZipStream(fs.createReadStream(program.archiveFile), options) :
...
function retrieve() {
  ...
  program.option('-D, --directory [directory]',
         ...
         .option('--memberTypes [memberTypes]', 'Metadata types and its members (e.g. ApexClass:Class1,Class2;ApexPage:Page1,Page2)')
         .option('--pollTimeout [pollTimeout]', 'Polling timeout in millisec (default is 60000ms)', parseInt)
         .option('--pollInterval [pollInterval]', 'Polling interval in millisec (default is 5000ms)', parseInt)
  acceptCommonOptions(program);
  program.parse(process.argv);
  ...
  if (!program.directory && !program.zipFile) {
    console.error('Error: Please set --directory or --zipFile option to specify the destination of retrieved metadata package');
    process.exit(1);
    return;
  }
  if (program.packageNames && program.directory && program.packageNames.length !== program.directory.length) {
    console.error('Error: Please set output directory paths in --directory option, corresponding to entiries in packageNames option')
    process.exit(1);
    return;
  }
  (
    ...
    Promise.reject(
      new Error('Please set --packageNames or --memberTypes in options, or speclify package.xml file path in --packageXML option')
    )
  )
  .then(function(res) {
    console.log('');
    ...
    if (!res.success) {
      ...
      console.log('');
      console.log('Retrieved metadata files are saved as a ZIP archive: ' + program.zipFile);
      return true;
