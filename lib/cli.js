...
function acceptConnectionOptions(program) {
  program.option('-u, --username [username]', 'Salesforce username')
         ...
         .option('-i, --instanceUrl [instanceUrl]', 'Salesforce login instance url')
         .option('-s, --sessionId [sessionId]', 'Salesforce Session id')
         .option('-sl, --serverUrl [serverUrl]', 'Salesforce Server Url')
...
function readOptions(program) {
  var options = {};
  "username,password,logLevel,accessToken,sessionId,serverUrl,instanceUrl,connection,loginUrl,pollInterval,pollTimeout".split(',').forEach(function(prop) {
